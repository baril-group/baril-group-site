...
(() => {
  ...
  const MAP = [
    ...
    ['#performance .lead', {
      nl: 'Gespecialiseerde coatings ontwikkeld om de doorlooptijd te verbeteren — minder uithardingstijd, minder afval en minder warmte nodig, zonder in te leveren op afwerkingskwaliteit. Gemaakt in Angola, Indiana, voor korte, betrouwbare levering.',
      pl: 'Wyspecjalizowane powłoki opracowane, by zwiększyć przepustowość — skracają czas utwardzania, ograniczają odpady i potrzebę ciepła, bez rezygnacji z jakości wykończenia. Produkowane w Angola, Indiana, dla krótkich i niezawodnych dostaw.',
      ro: 'Vopsele specializate concepute pentru a îmbunătăți productivitatea — reduc timpii de uscare, deșeurile și nevoia de căldură, fără a renunța la calitatea finisării. Fabricat în Angola, Indiana, pentru o aprovizionare scurtă și fiabilă.',
      de: 'Spezialisierte Beschichtungen, entwickelt für mehr Durchsatz — sie reduzieren Trocknungszeiten, Abfall und den Wärmebedarf, ohne Abstriche bei der Finish-Qualität. Hergestellt in Angola, Indiana, für kurze, verlässliche Lieferung.',
      fr: 'Des revêtements spécialisés conçus pour améliorer la cadence — en réduisant les temps de séchage, les déchets et le besoin de chaleur, sans rien céder sur la qualité de finition. Fabriqué à Angola, Indiana, pour un approvisionnement court et fiable.',
      es: 'Recubrimientos especializados diseñados para mejorar la productividad — reduciendo los tiempos de curado, los residuos y la necesidad de calor, sin renunciar a la calidad del acabado. Fabricado en Angola, Indiana, para un suministro corto y fiable.',
      it: 'Rivestimenti specializzati progettati per migliorare la produttività — riducendo tempi di essiccazione, sprechi e necessità di calore, senza rinunciare alla qualità della finitura. Prodotto ad Angola, Indiana, per una fornitura breve e affidabile.',
      tr: 'İş akışını iyileştirmek için tasarlanmış uzman kaplamalar — finiş kalitesinden ödün vermeden kürlenme sürelerini, israfı ve ısı ihtiyacını azaltır. Angola, Indiana\'da üretilir, kısa ve güvenilir tedarik için.', no: 'Spesialiserte belegg utviklet for å bedre gjennomstrømningen — mindre herdetid, mindre avfall og mindre varme nødvendig, uten å gå på akkord med finishkvaliteten. Laget i Angola, Indiana, for kort og pålitelig levering.', sv: 'Specialiserade beläggningar utvecklade för att förbättra genomströmningen — mindre härdtid, mindre avfall och mindre värme som behövs, utan att tumma på finishkvaliteten. Tillverkad i Angola, Indiana, för kort och pålitlig leverans.' }],
